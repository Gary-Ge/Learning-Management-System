...
export default function IndexPage() {
  ...
  const getEndTime = (item: any) => {
    return item.assignment_end || item.quiz_end;
  };
  ...
  const getCalendar = () => {
    ...
    .then(res => {
      ...
      let first_three_due:any = []
      let all_due:any = []
      get_all_due.AssignmentList.map((item:any)=> {
        all_due = all_due.concat(item)
        first_three_due = first_three_due.concat(item)
      })
      ...
      first_three_due.sort((a: any, b: any) => {
        const timeA = getEndTime(a);
        const timeB = getEndTime(b);
      
        return timeA.localeCompare(timeB);
      });
      const newdata = first_three_due.slice(0, 3).map((item: any) => {
        const date = new Date(getEndTime(item)); 
...
const handleTitleClick = (courseid:any, title:any) => {
  const encodedTitle = encodeURIComponent(title);
  const url = `http://localhost:8000/studentcourse?courseid=${courseid}&title=${encodedTitle}`;
            ...
            <List.Item>
              <List.Item.Meta
                ...
                title={<span
                  ...
                  onClick={() => handleTitleClick(item.courseid, item.title)}
                >
                  {item.title}
                </span>}
